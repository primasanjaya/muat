...
class MuAtDataloader(Dataset):
    ...
    def get_data(self, idx):
        ...
        for key,value in avail_count.items():
            if avail_count[key] > 0:
                if key == 'SNV':
                    pd_samp = pd_row[pd_row['mut_type'] == 'SNV'][grab_col]
                    pd_samp = pd_samp.sample(n = avail_count[key], replace = False)
                    pd_sampling = pd.concat([pd_sampling, pd_samp], ignore_index=True)
                elif key == 'MNV':
                    pd_samp = pd_row[pd_row['mut_type'] == 'MNV'][grab_col]
                    pd_samp = pd_samp.sample(n = avail_count[key], replace = False)
                    pd_sampling = pd.concat([pd_sampling, pd_samp], ignore_index=True)
                elif key == 'indel':
                    pd_samp = pd_row[pd_row['mut_type'] == 'indel'][grab_col]
                    pd_samp = pd_samp.sample(n = avail_count[key], replace = False)
                    pd_sampling = pd.concat([pd_sampling, pd_samp], ignore_index=True)
                elif key == 'SV/MEI':
                    pd_samp = pd_row[pd_row['mut_type'] == 'SV/MEI'][grab_col]
                    pd_samp = pd_samp.sample(n = avail_count[key], replace = False)
                    pd_sampling = pd.concat([pd_sampling, pd_samp], ignore_index=True)
                elif key == 'Neg':
                    pd_samp = pd_row[pd_row['mut_type'] == 'Neg'][grab_col]
                    pd_samp = pd_samp.sample(n = avail_count[key], replace = False)
                    pd_sampling = pd.concat([pd_sampling, pd_samp], ignore_index=True)
        ...
        is_padding = False
        if len(pd_sampling) < self.mutation_sampling_size:
            mins = self.mutation_sampling_size - len(np_triplettoken)
            is_padding = True
            
        datanumeric = []
        
        for i in pd_sampling.columns:
            np_data = pd_sampling[i].to_numpy() 
            if is_padding:
                np_data = np.copy(np.pad(np_data, ((0, mins)), mode='constant', constant_values=0))
            if len(np_data) > self.mutation_sampling_size:
