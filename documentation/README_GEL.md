...
```bash
(muat-env)$ muat predict from-checkpoint --hg38 '/re_gecip/machine_learning/muat/genome_ref/ref38_fa' --ckpt-filepath 'path/to/checkpoint.pthx' --input-filepath 'sample1.vcf.gz' --result-dir 'path/to/result_dir/'
```

## Quick VCF hg19 Inference
```bash
(muat-env)$ muat predict from-checkpoint --hg19 '/re_gecip/machine_learning/muat/genome_ref/ref' --ckpt-filepath 'path/to/checkpoint.pthx' --input-filepath 'sample1.vcf.gz' --result-dir 'path/to/result_dir/'
```

## Quick Inference from Preprocessed Data (`.muat.tsv` / `.muat.tsv.gz`)
```bash
(muat-env)$ muat predict from-checkpoint --ckpt-filepath 'path/to/checkpoint.pthx' --input-filepath 'sample1.muat.tsv' --result-dir 'path/to/result_dir/'
```
There is no `--no-preprocessing` flag any more: muat decides whether an input is raw or
already preprocessed from its **suffix** alone. Accepted suffixes are `.vcf{,.gz}`,
`.maf{,.gz}` and `.tsv` (raw, and these require `--hg19`/`--hg38`), or `.muat.tsv{,.gz}`
(preprocessed). Do **not** pass a reference with a preprocessed input — that is now an error.

If you still hold files named `*.token.gc.genic.exonic.cs.tsv.gz` from an earlier muat
version, rename or symlink them to `*.muat.tsv.gz`. It is the same tokenized table, so no
re-preprocessing is needed — only the suffix is inspected.
...
```bash
muat preprocess --somagg --hg38 'path/to/ref_38.fa' --input-filepath 'path/to/somagg/chunks.vcf.gz' --tmp-dir 'path/to/preprocessed_data/'
```
...
```
This will generate a file named `platekey_sample.muat.tsv`.
...
| :------------------- | :---------:| :------------: | :---------: | --------------:|
| platekey1.muat.tsv   |   BREAST   | DUCTAL         | 1           |  13            |
| platekey2.muat.tsv   |   LUNG     | ADENOCARCINOMA | 2           |  10            |

For MuAt2e models where the prediction heads are combined type/typesubtype you can put it like below:

| prep_path            | class_name |    subclass_name    | class_index | subclass_index |
| :------------------- | :---------:| :------------------:| :---------: | --------------:|
| platekey1.muat.tsv   |   BREAST   | BREAST DUCTAL       | 1           |  76            |
| platekey2.muat.tsv   |   LUNG     | LUNG ADENOCARCINOMA | 2           |  15            |
